        ...
        <section className="relative bg-black bg-opacity-90 text-white py-12 px-6 flex flex-col items-center" role="region">
            <div className="w-full flex justify-between items-center mt-5">
                ...
                <button className="text-gray-400 text-sm hover:text-white mr-10"
                        onClick={()=>{handleTrend()}}>더보기</button>
            </div>
            ...
            <div className="relative w-full px-11 mt-6 mb-10 swiper-container-group">
                <Swiper
                    ...
                    {reviews.map((item, index) => {
                        ...
                        const roundedClass = [2, 6, 10].includes(index) ? "rounded-full" : "rounded-lg";
                        return (
                            <SwiperSlide key={item.id} className={`relative w-[400px] h-[420px] ${roundedClass} shadow-lg`}>
                                <div className={`relative w-full h-[420px] overflow-hidden ${roundedClass} shadow-lg cursor-pointer`}
                                     onClick={()=>{handleTrend(item.id)}}>
                                    <Image
                                        // src={item.src}
