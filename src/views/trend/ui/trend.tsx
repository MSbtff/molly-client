...
export default function Trend() {
    ...
    const fetchTrendingReviews = async () => {
        try {
            ...
            const productPromises = reviewData.map((review: typeof reviewData[number]) =>
                getProduct(`${baseUrl}/product/${review.productId}`).catch(() => null)
            );
            const products = await Promise.all(productPromises);

            // 리뷰와 상품 데이터를 결합
            const combinedData = reviewData.map((review: typeof reviewData[number], idx: number) => ({
                ...review,
                ...
                {reviews.map((item) => (
                    <div key={item.id} id={`review-${item.id}`} className="pb-6">
                        ...
                        <div className="flex items-center mb-4">
                            <Image src="/logo.webp" alt="profile" width={40} height={40} className="rounded-full" />
                        ...
                        {item.product && (
                            ...
                            >
                                ...
                                <div className="ml-3">
                                    ...
                                    <p className="text-gray-500">{item.product.price.toLocaleString()}원</p>
                                </div>
                            </div>
                        )}
                        {/* 리뷰 내용 */}
